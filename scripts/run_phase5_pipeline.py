...
Stage 3 uses existing shards and automatically selects its completed Stage 2
baseline; it never launches collection or repeats the earlier stages.
"""
...
def main() -> None:
    ...
    if args.stage != "stage3" and not args.skip_collection:
    ...
    if args.selected_action_weight is not None:
        command.extend(["--selected-action-weight", str(args.selected_action_weight)])
    if args.dry_run:
